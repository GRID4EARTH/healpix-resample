...
CNRS--IFREMER--IRD--Univ. Brest--IUEM, Plouzané, France.
Email: camille.gueguen@ifremer.fr. ORCID: 0000-0000-0000-0000.}%
...
\begin{abstract}
Earth-observation products are commonly distributed on projected grids,
whereas global analyses increasingly use hierarchical spherical grids such
as \healpix. Standard interpolation ignores the sensor point-spread
function (\psf) and can distort spatial structure when the target support is
finer than the source sampling. We present \texttt{healpix-resample}, a
sparse framework that models projected-grid pixels as spatially weighted
observations of a latent \healpix cell-average field. Two dual-normalized
transfer operators form an exact adjoint pair under grid-dependent weighted
inner products, and a damped correction is obtained with a matrix-free,
weighted conjugate-gradient solve on GPU. In a controlled synthetic
benchmark over 40 regions, the nominal Gaussian-width reconstruction reduced
blur-induced RMSE by 15.4--19.6\% across four scene classes. In a
reduced-resolution validation using real Sentinel-2 B04 imagery, the method
achieved the lowest RMSE in all 40 regions against each of four competitors,
with gains of 11.0--17.9\% over Richardson--Lucy, the strongest comparator.
The ordering was unchanged with an estimator-independent geometric
reference. It was largely preserved for 20\% response-width errors, with one
smooth-scene exception for a 20\% overestimate. Median operator construction
plus solution time was 0.37~s for a $256\times256$ patch on an NVIDIA L4.
The method provides model-consistent transfer to \healpix when spatial
response and geolocation are sufficiently accurate, and supplies a basis for
future multi-observation map-making.
\end{abstract}
...
We introduce \texttt{healpix-resample}, a \psf-aware framework for mapping projected EO imagery onto \healpix (Fig.~\ref{fig:workflow}). Rather than interpolating pixel values onto HEALPix cells, we ask the reverse question: what HEALPix field, once observed through the sensor's spatial response at the pixels' actual locations, would reproduce the recorded image? Answering it requires two sparse operators — a forward model and its weighted adjoint — and a matrix-free least-squares solve on GPU (Section III). Only sample geolocations and a local response model are needed; the released interface and the validation here use a scalar isotropic Gaussian and Sentinel-2 B04.

The validation is organized around two recovery benchmarks and one
auxiliary consistency check. First, a controlled synthetic-detector
experiment uses Esri World Imagery as a high-resolution spatial texture,
convolves it with a prescribed Gaussian response, and compares the
reconstructed field with the pre-degradation texture
\cite{esri_world_imagery}. Second, a controlled reduced-resolution experiment
using real Sentinel-2 imagery
(Section~\ref{sec:real_downscale_experiment}) applies a known degradation to
native B04 pixels and evaluates all methods against a reference constructed
before that degradation. This tests reconstruction on real radiometry under a
known forward model; it is not an external higher-resolution reference or an
independent calibration of the instrumental \psf. Both recovery experiments
are extended to 40 geographically distributed regions. Separately, a
four-patch \utm--\healpix--\utm round trip is retained only as an
implementation and observation-consistency sanity check, because the same
operator enters the fit and the prediction.
...
The contribution is threefold. First, projected-grid pixels are posed as
sparse observations of a \healpix cell-average field through an explicit
spatial-response model, rather than treated by geometry alone. Second, two
dual-normalized operators form an exact weighted adjoint pair and permit a
matrix-free GPU solve with linear per-iteration cost for fixed neighborhood
size. Third, the formulation is validated on controlled synthetic imagery
and real Sentinel-2 data over 40 geographically distributed regions, with
geometric and deconvolution baselines, response-mismatch tests, and an
estimator-independent reference. The open-source implementation is designed
for \healpix/DGGS data-cube workflows \cite{healpix_resample_repo}.

Detailed robustness, throughput, and area-integral-conservation results are
